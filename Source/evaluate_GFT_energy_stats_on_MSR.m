%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%% This is the script to evaluate spatial GFT (with pre-determined spatial graph) 
%%% + energy distribution statistics features 
...
% specify feature type: 1:raw energy distribution; 2:stats(med&std) of
% coeff amplitude dist. in seq
featureType = 2; 
    ...
    for action = 1:n_actions
        for instance = 1:n_instances
            if (skeletal_data_validity(action, subject, instance))                    
                ...
                if featureType==1
                    % calculate energy distribution ratio over GFT basis (each dimension respectively)
                    energyGFT = sum(GFT_coeff.^2, 1);
                    energyDistb = energyGFT ./ sum(energyGFT);
                    features{count} = energyDistb;
                elseif featureType==2
                    % calculate stats(med&std) of coeff amplitude dist. in seq
                    energyGFT = GFT_coeff.^2;
                    energyDistb = energyGFT ./ repmat(sum(energyGFT,2), 1, size(energyGFT,2));
                    features{count} = [median(energyDistb,1), std(energyDistb,0,1)];
...
if adoptPCA==1
    ...
    numCP = min(find(cumdist >= 99.9));
    train_FeatureSet = SCORE(:, 1:numCP);
    temp = test_FeatureSet - repmat(MU, size(test_FeatureSet,1), 1);
    test_FeatureSet = temp*COEFF(:, 1:numCP);
end
